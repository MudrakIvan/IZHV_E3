...
public class Gun : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (mFiring && fireEnabled)
        {
            ...
            { // Spawn corresponding number of bullets.
                ...
                // "Heat up" the gun.
                mCoolDown += shotgun ? 2 * secondsPerBullet : secondsPerBullet;
            }
        }
    }
    ...
    public void ShootGun(Transform director)
    {
        ...
        // spawn bullets in shotgun spread
        for (float spread = - (shotgunSpread / 2); spread <= shotgunSpread / 2; spread += shotgunSpread / (shotgunBullets - 1)){
            var bulletPosition = director.position;
            var eulerAngles = director.eulerAngles;
            // add spread to position
            eulerAngles.y += spread;
            var bulletRotation = Quaternion.Euler(eulerAngles);
    ...
    /// Spawn a single bullet using provided transform as a director.
    /// </summary>
    public void SpawnBullet(Vector3 position, Quaternion rotation, bool shotgun = false)
    {
        if (mUseECS)
